...
app.listen(8080, function () {
    console.log('listening on *:8080  - WebSocketServer is running');
});
...
io.on('connection', function (socket) {
    ...
    socket.on('order_created', order => {
        if (order) {
            ...
            io.sockets.emit('order_created', order);
        }
    });
    socket.on('order_terminated', order => {
        if (order) {
            // socket.broadcast.emit('order_created', order);
            io.sockets.emit('order_terminated', order);
        }
    });
    socket.on('order_prepared', order => {
        if (order) {
            // socket.broadcast.emit('order_created', order);
            io.sockets.emit('order_prepared', order);
        }
    });
    socket.on('meal_created', order => {
        if (order) {
            // socket.broadcast.emit('order_created', order);
            io.sockets.emit('meal_created', order);
        }
    });
    socket.on('invoice_created', (meal) => {
        // socket.broadcast.emit('order_created', order);
    ...
    socket.on('user_deleted', (user) => {
        io.sockets.emit('user_deleted', user);
    });
});
